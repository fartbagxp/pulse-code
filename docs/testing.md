...
Unit tests cover catalog and matcher lookups, XML template merging (including a regression test for the CDC WONDER radio-button trap), AAR constraints, provider selection, and every CLI command that works offline.

Integration tests live in `tests/integration/` and are excluded by default. There are two of them, and they behave differently.
